...
<p class="block text-3xl font-bold mb-5 text-center text-[#f85a38]">Trình chỉnh sửa video✂️</p>
    ...
    <div class="flex flex-col h-full w-[350px] border-[1px] rounded-lg p-5 pt-2">
    	 ...
    	 <div class="flex flex-col gap-2">
	    	 <c:forEach var="item" items="${myData}">
	    	  <c:choose>
				  ...
				  <c:when test="${item.data != 'processing' && item.data != 'error'}">
      ...
      <div class="relative w-[940px] h-[464px]">
        <div id="require-select" class="w-full h-full absolute top-0 left-0 bg-[#dce7f5] z-10 flex
          justify-center items-center text-lg font-medium">Vui lòng chọn video💻</div>
      ...
      <p class="text-lg my-5 font-bold text-[#f85a38]">Chọn khoảng thời gian bạn muốn cắt🪄</p>
       <div class="flex flex-col gap-2">
        <div class="flex gap-3">
          <div class="button bg-[#5c7099] w-fit text-white px-2 py-1 rounded-md cursor-pointer" onClick="getValues()">Lấy thời gian đã chọn</div><br/>
       </div>
        ...
        <input type="file" 
	      ...
	      <div>
			<label for="mute">Mute audio</label>
	     	<input type="checkbox" name="isMute" id="mute">
		  </div>
